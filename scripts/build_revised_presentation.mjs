...
async function main() {
  ...
  const output = path.resolve(
    args.output || path.join(root, "tmp", "revision", "candidate.pptx"),
  );
  const previewDir = path.resolve(
    args["preview-dir"] || path.join(root, "tmp", "revision", "preview"),
  );
  const chartAssetDir = path.resolve(
    args["chart-asset-dir"] || path.join(root, "tmp", "revision", "chart_assets"),
  );
  const { Presentation, PresentationFile } = await loadArtifactTool();
  for (const directory of [previewDir, chartAssetDir]) {
    const relative = path.relative(path.join(root, "tmp", "revision"), directory);
    if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) {
      throw new Error(`Renderer scratch directories must be children of tmp/revision: ${directory}`);
    }
  }
  ...
  ], SLIDE_STYLE[7], path.join(root, "outputs/revision/ranking_plotly.png"), "WEIGHTED AVERAGE");
  ...
  if (!args["skip-preview"]) await writeBlob(
...
function addCover(deck, summary, slideData) {
  ...
  addText(slide, "Yield as a regime, not a ranking.", 56, 488, 620, 38, {
    fontSize: 24,
    bold: true,
    color: COLORS.coral,
  });
  addText(slide, "Simone Ragusini  ·  945119", 56, 551, 645, 30, { fontSize: 23, bold: true, color: COLORS.white });
  addText(slide, "s.ragusini@campus.unimib.it", 56, 588, 645, 28, { fontSize: 21, color: "#DDE6FF" });
...
function addResearchFrame(deck, slideData) {
  ...
  addText(
    slide,
    "APY = quoted annualized yield. TVL = total value locked, measured in USD.",
    44,
    148,
    1010,
    42,
...
function addEventResponse(deck, summary, slideData, apyChartAsset, tvlChartAsset) {
  ...
  addText(slide, "APY declines while observed TVL rises", 44, 80, 1020, 60, {
    fontSize: 40,
    bold: true,
  });
...
function addConclusion(deck, summary, slideData) {
  ...
  addText(
    slide,
    "Code: github.com/Laimon99/stablecoin-yield-visualization\nMatplotlib + Plotly. Code: MIT. Original presentation: CC BY 4.0.",
    82,
    563,
    1040,
    62,
...
function addToolsAndEvidence(deck) {
  const slide = baseLightSlide(deck, 15, null, COLORS.blue, "TOOLS / VISUAL DESIGN");
  addText(slide, "Two visualization tools, complementary uses", 44, 88, 1160, 60, {fontSize: 39, bold: true});
...
function setSpeakerNotes(slide, slideData) {
  if (slideData?.speaker_note) {
    slide.speakerNotes.textFrame.setText(slideData.speaker_note.replace("Use the native chart.", "Use the chart."));
